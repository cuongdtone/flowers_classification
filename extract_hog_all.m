...
% set path data
path_data = 'D:\ky_thuat_nhan_dang\datasets\dataset_gray\';
% set path save
...
number_feature = ((64-size_window)/stride + 1)*((64-size_window)/stride + 1)*36;
...
for k = 1:length(class)
    ...
    for i=1:length(list_file)
        ...
        if length(size(im_org)) > 2
            im_org = rgb2gray(im_org);
        end
        % extract hog
        feature_hog = extract_hog(im_org, [64, 64]);
